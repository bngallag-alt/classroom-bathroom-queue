...
Weekly bathroom-time usage is derived from counted completed sessions within teacher-selected local calendar weeks; no separate running counter is stored. Teachers can optionally deduct overtime beyond a configured grace period from the immediately following week's allowance. Weekly limits, next-week deductions, overtime-count warnings, and rolling automatic suspensions are disabled by default. Teachers can configure individual weekly allowances or unlimited time and can apply, adjust, or end temporary suspensions with PIN confirmation. These controls affect only the app's pass workflow and are not a substitute for emergency or accommodation access.

...
export function validateWeeklyTimePolicy(input: WeeklyTimePolicy, automaticSuspensionAcknowledged: boolean) {
  ...
  if (!Number.isInteger(policy.overageGraceMinutes) || policy.overageGraceMinutes < 0) return 'Allowed overage must be a nonnegative whole number of minutes.';
  if (!Number.isInteger(policy.suspensionDays) || policy.suspensionDays < 1 || policy.suspensionDays > 30) return 'Weekly suspension duration must be a whole number from 1 through 30 days.';
...
export function effectiveWeeklyAllowance(student: Student, settings: Settings) {
  const policy = normalizeWeeklyTimePolicy(settings.weeklyTimePolicy);
  if (!policy.enabled) return { mode: 'disabled' as WeeklyAllowanceMode, allowanceSeconds: undefined };

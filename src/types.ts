...
export type UsageLimitMode='bathroom-passes'|'weekly-time';
export type ProbationMetric='uses'|'overtimeCount'|'overtimePercent';
export type ProbationRule={enabled:boolean;warningThreshold:number;suspensionThreshold:number};
export type ProbationPolicy={warningsEnabled:boolean;automaticSuspensionsEnabled:boolean;automaticSuspensionAcknowledged:boolean;suspensionDays:number;minimumUsesForPercent:number;triggerMode:'any'|'all';rules:Record<ProbationMetric,ProbationRule>;updatedAt?:string};
export type Weekday=0|1|2|3|4|5|6;
export type WeeklyTimePolicy={enabled:boolean;allowanceMinutes:number;resetDay:Weekday;warningRemainingMinutes:number;deductOvertimeNextWeek:boolean;overtimeGraceMinutes:number;automaticSuspensionEnabled:boolean;overageGraceMinutes:number;suspensionDays:number;updatedAt?:string};
export type PassSuspension={kind:'automatic'|'manual';source?:'rolling-30-day'|'weekly-time'|'manual';startedAt:string;endsAt:string;reasons:string[];triggerKey?:string};
export type Student={id:string;name:string;externalIdHash:string;externalIdLast4:string;createdAt:string;updatedAt:string;deletedAt?:string;organizationId?:string;schoolId?:string;maxUses?:number|null;banned?:boolean;probationExempt?:boolean;weeklyTimeLimitMinutes?:number|null;passSuspension?:PassSuspension;lastAutoSuspensionTriggerKey?:string;lastWeeklyTimeSuspensionTriggerKey?:string};
...
export type Settings={id:'settings';warningSeconds:number;overLimitSeconds:number;pinHash:string;pinSalt:string;idDisplay:'masked'|'full'|'name';schemaVersion:number;setupComplete:boolean;usageLimitMode:UsageLimitMode;globalUseLimit:number;countWaterAsBathroom:boolean;teacherHotkey:string;clearQueueHotkey:string;currentClassId?:string;activeScheduleId:string;todayScheduleId?:string;todayScheduleDate?:string;autoSchedulePausedDate?:string;manualOverrideBlockId?:string;lastScheduleStateId?:string;clockWarning?:string;lookupSecret:string;privacyVersion:number;privacyMigrationCompletedAt?:string;privacyNoticeAcknowledged:boolean;historyRetentionDays:number;lastSuccessfulPurgeAt?:string;probationPolicy:ProbationPolicy;weeklyTimePolicy:WeeklyTimePolicy;lastSeenReleaseId?:string};
